  ...
  app.post('/updateProduct', upload.single('myFile'), async(req, res, next) => {  
    ...
    if (!file) {      
      console.log("no image found")
       savedImage=await productModel.findByIdAndUpdate({_id:id},{
        name:name,
        price:price,
        quantity:quantity
      })
   ...
   app.post('/updatePortfolio', upload.single('myFile'), async(req, res, next) => {  
    try{  
    const file = req.file    
    console.log("at file")
    console.log(req.body)
    ...
    if (!file) {      
      console.log("no image found")
       savedImage=await portfolioModel.findByIdAndUpdate({_id:id},{
        name:name,
